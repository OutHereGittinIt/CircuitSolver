...
%% Brainstorming
%
% I think we need to redo our idea for solving the circuit
% Maybe not entirely but in some crucial ways
% We want to draw node side with two noted voltages 
% The indices for these voltages in the equation matrix are gonna be
% denoted by their Node Index

% But that has me thinking, shouldnt we do that for every side?

% Child sides are the only sides that would be different (dont have
% existing node references)

% Child sides have current split and side countts with specific remembered
% voltages
%   That's all fine and squared away

% We made count side to have an ending and the beggining is just voltage_ind

% SO, when we run into Node Side, we add voltage equation and then
% associate the node with new voltage_ind, stored away in a NEW list
% initialized to zeros. lets say its called nodevoltages
%   nodevoltages(node_ind) = voltage_ind (<-- new)

% So we go around the entire circuit labeling these. THEN, after the
% circuit, we count our NODE sides, but with these two differences :
%   - begin by setting voltage_ind to the voltage_ind of 'from' node
%   - use voltage_ind of 'to' node as function input

% This is somewhere where isOriginal is useful


% settings vs. general settings but can I actually do this?? Have to remove
% the plot options from the general options display (easy enough -- make a
% group like the others and determine that that group is not displayed. So
% you dont need to update to reflect. But it is a seperate save file than
% the general settings. Should they be part of the options?
% we can return the options group from the settings tab.
%
% OK so i have a given circuit plot, I can easily edit this group of
% settings an reapply with redraw_square(f,new_opts)
%
% But new_opts now has to be saved to mat file and the next time the
% settings button pushed that file is loaded to display options
%
% What I'm getting at is it does seem like the best thing to do here is
% separate options from plot options. It's a lot of find and replace but
% thats fine.
%
% The main difference btween the two is that one ca be applied with a
% redraw square and the other one is applied with close(f) and create_GUI
%
% When we do aplly general setting, there is going to be drawing involved
% immediately, so we need to pull the latest plot options as well as the
% general options. It does seem like the best practice for that to happen
% smoothly is to have two options variables, but then do these need to be
% fed seperately  to every function?? Cause that's actually a little
% annoying.
%
% So this gets interesting for sure. I dont really want two options
% variables to throw in every where becuase redraw_side is called *many*
% places
%
% One opts is good. So the question becomes a matter of updating and
% passing properly. So settings can be edited by user in two places :
%
% - plot sub-gui    --> redraw_square
%       Change, update, redraw with new options, PASS NEW OPTIONS (save)
%       BUT where do we load these options? The functions are created with
%       opts input. There's no way you can redo them without recreating the
%       function calls
%
%       Ok, here's someting. Is redraw_side the ONLY call that uses these
%       opts? I think so, no actually the element plot shapes are generated at the very
%       beggining and stored to user data, so you wald have to redo that
%       with new options. Thta part is actually pretty easy if you do it in
%       the apply setting function call. 
%
%       So outside of that, you would have to edit redraw_side to look for 
%       a saved file for options to import. That should be everything you
%       need to edit. 
%
%       The idea here is its nice to redo the circuit draw without
%       recreating GUI. So far I think its doable but I'm not sure if its
%       really worth the trouble.
%
%       The more I think about it, the unfortunate truth is we need to get
%       rid of the setting panel I think. And do a similar thing with the
%       plot circuit. thats is a SHAME because I put a lot of work into
%       that lol. 
%
%       The plot settings sub gui is definitely superior. We can take a lot
%       of the ideas but there is a lot of new typing needed for sure. 
%
%       Reading the saved file for options is not a bad idea. Will probably
%       add a lot of time though if you need to read the optoins for every
%       side lol. Ok actually thats a horrible idea. The opts need to be
%       set somehow. You could ALMOST put the opts in userdata but god that
%       is a LOT more typing space and could get really ugly. 
%
%       I guess referring to opts as a hardcoded variable isnt great.
%       Because we have to recreate everything with a new variable. If it
%       was part of UserData that would be annoying but also would be
%       easier to reference when updated.
%
%       At the end of all of this, I am UNSURE. So hows that. We'll keep
%       thinking about it. I think we are pretty clear on the superior
%       outcome, but not sure about how to get it done best.
%
%       So we definietly dont want to read opts every side draw. That is
%       retarded, excuse my language. It needs to be referenced somewhere.
%       Where would that be? 
%
%       I guess it could be f.UserData.opts.x
%           ^ Althought that is a really substantial shift from what we
%           were doing and will not be easy to do. 
%       The other option is read from file, which we didnt like. The last
%       option is to recreate the GUI, read options from file ONE TIME and
%       pass that variable on. 
%
%       Here's an example of re-using those new_options. Add a new element
%       to existing line. Now we redraw the line. Let's just say that we
%       changed LineWidth, for example. If you reset opts your fine for
%       place_element, but you're bad for restore_prev_sides, for example.
%       You could go and change the function call there.
%
%
%
%
%       OK. I am oficially depressed. The best thing I can do is have a fun
%       challenge to distract myself in this time. I also need to work out,
%       drink lots of water, and maybe later I can start thinking about my
%       email I'm gonna send eventually. They're not gonna take me back.
%       But do what you can.
%
%
%       I think we've been defeated in this one. I can't think of a way
%       unless options are from f.UserData. In the case of adding new
%       element,it actually works out becuase once you plot with new opts,
%       those new opts will be referenced in place_element on the line.
%
%       I guess the best option I can think of is save opts to UserData,
%       and then in redraw_side, if opts is a field in userdata use that
%       one, otherwise use provided. That way you dont have to read from
%       file everytime, which you will still save.
%
%       ^ for some reasons this seems almost like a cheat or something but
%       I think its actually OK.
%
% - general tab     --> create_GUI
%
% If we keep them together, then we need to read from two files.  


% ^ This brings up another interesting idea. If I was to create a host of
% different apps, I think I should do what I was heading with USFalcon
% (sad!) with general app building tools. 
%
% Add file prompt (done there, sad), saving/loading, settings changing and
% saving, should all have a lot of common functionality with varying
% inputs. hmmmmmmmmmmm. Maybe for another time? Idk. Its a fun idea. We
% want to do fun ideas. That's the whole point. There is no real use case
% here. 

% Maybe one day I'll retrun there. December 10th. Workout and drink water. 
